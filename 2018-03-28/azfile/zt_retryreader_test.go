...
type retryReaderSuite struct{}

var _ = chk.Suite(&retryReaderSuite{})
...
func (r *retryReaderSuite) TestRetryReaderReadWithRetry(c *chk.C) {
...
func (r *retryReaderSuite) TestRetryReaderReadNegativeNormalFail(c *chk.C) {
	...
	retryReader := NewRetryReader(ctx, &startResponse, HTTPGetterInfo{Offset: 0, Count: int64(byteCount)}, rrOptions, getter)
...
func (r *retryReaderSuite) TestRetryReaderReadCount0(c *chk.C) {
	byteCount := 1
	body := newPerByteReader(byteCount)
	body.doInjectError = true
	...
	retryReader := NewRetryReader(ctx, &startResponse, HTTPGetterInfo{Offset: 0, Count: int64(byteCount)}, RetryReaderOptions{MaxRetryRequests: 1}, getter)
...
func (r *retryReaderSuite) TestRetryReaderReadNegativeNonRetriableError(c *chk.C) {
	byteCount := 1
	body := newPerByteReader(byteCount)
	body.doInjectError = true
	body.doInjectErrorByteIndex = 0
	...
	retryReader := NewRetryReader(ctx, &startResponse, HTTPGetterInfo{Offset: 0, Count: int64(byteCount)}, RetryReaderOptions{MaxRetryRequests: 2}, getter)
...
func (r *retryReaderSuite) TestRetryReaderReadWithForcedRetry(c *chk.C) {
