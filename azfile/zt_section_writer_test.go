...
func (s *FileURLSuite) TestSectionWriter(c *chk.C) {
	b := [10]byte{}
	buffer := azfile.NewBytesWriter(b[:])

	section := azfile.NewSectionWriter(buffer, 0, 5)
	...
	section = azfile.NewSectionWriter(buffer, 5, 6)
...
func (s *aztestsSuite) TestSectionWriterCopySrcDestEmpty(c *chk.C) {
	...
	output := make([]byte, 0)
	buffer := azfile.NewBytesWriter(output)
	section := azfile.NewSectionWriter(buffer, 0, 0)
